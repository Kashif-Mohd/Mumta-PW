...
namespace maamta_pw
{
    public partial class showcrf5a : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            try
            {
                if (CheckBox1.Checked == false)
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf5a WHERE DSSID LIKE '%" + txtdssid.Text + "%' and (str_to_date(pw_crf5a_02, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y'))", con);
                ...
                else
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf5a WHERE DSSID LIKE '%" + txtdssid.Text + "%' ", con);
                    MySqlDataAdapter sda = new MySqlDataAdapter();
                    {
                        cmd.Connection = con;
                        sda.SelectCommand = cmd;
        ...
        protected void Link_StudyID(object sender, EventArgs e)
        {
            ...
            string form_crf_5a_id = commandArgs[0];
            string study_code = commandArgs[1];

            Session["form_crf_5a_id"] = form_crf_5a_id;
            Session["StudyIdCRF5a"] = study_code;
            Session["BackButton"] = "showcrf5a";
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=CRF5a (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
