...
namespace maamta_pw
{
    public partial class showanc : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            try
            {
                if (CheckBox1.Checked == false)
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_anc WHERE DSSID LIKE '%" + txtdssid.Text + "%' and (str_to_date(date_of_attempt, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y'))", con);
                ...
                else
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_anc WHERE DSSID LIKE '%" + txtdssid.Text + "%' ", con);
                    MySqlDataAdapter sda = new MySqlDataAdapter();
                    {
                        cmd.Connection = con;
                        sda.SelectCommand = cmd;
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=ANC-Form (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
