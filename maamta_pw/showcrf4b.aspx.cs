...
namespace maamta_pw
{
    public partial class showcrf4b : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            try
            {
                if (CheckBox1.Checked == false)
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf4b WHERE DSSID LIKE '%" + txtdssid.Text + "%' and (str_to_date(pw_crf4b_2, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y'))", con);
                ...
                else
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf4b WHERE DSSID LIKE '%" + txtdssid.Text + "%' ", con);
                    MySqlDataAdapter sda = new MySqlDataAdapter();
                    {
                        cmd.Connection = con;
                        sda.SelectCommand = cmd;
        ...
        protected void Link_StudyID(object sender, EventArgs e)
        {
            ...
            string form4b_id = commandArgs[0];
            string study_code = commandArgs[1];

            Session["form_crf_4b_id"] = form4b_id;
            Session["StudyIdCRF4b"] = study_code;
            Session["BackButton"] = "showcrf4b";
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=CRF4b (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
