...
namespace maamta_pw
{
    public partial class showcrf5c : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            try
            {
                if (CheckBox1.Checked == false)
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf5c WHERE DSSID LIKE '%" + txtdssid.Text + "%' and (str_to_date(pw_crf5c_2, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y'))             order by str_to_date(pw_crf5c_2, '%d-%m-%Y'), STR_TO_DATE(pw_crf5c_3,  '%H:%i')", con);
                ...
                else
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf5c WHERE DSSID LIKE '%" + txtdssid.Text + "%'              order by str_to_date(pw_crf5c_2, '%d-%m-%Y'), STR_TO_DATE(pw_crf5c_3,  '%H:%i')", con);
                    MySqlDataAdapter sda = new MySqlDataAdapter();
                    {
                        cmd.Connection = con;
                        sda.SelectCommand = cmd;
        ...
        protected void Link_StudyID(object sender, EventArgs e)
        {
            ...
            string form_crf_5c_id = commandArgs[0];
            string study_code = commandArgs[1];

            Session["form_crf_5c_id"] = form_crf_5c_id;
            Session["StudyIdCRF5c"] = study_code;
            Session["BackButton"] = "showcrf5c";
            Response.Redirect("showcrf5cbyid.aspx");
        }
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=CRF5c (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
