...
namespace maamta_pw
{
    public partial class CholinePrtage : System.Web.UI.Page
    {
        ...
        {
            string query = string.Format("SELECT 'Greater than 75.0%' AS FIELD, COUNT(c.study_code) AS 'total'				FROM 		pregnant_woman AS a LEFT JOIN  studies AS c ON c.pw_id=a.pw_id LEFT JOIN form_crf_3a AS d ON d.pw_id=a.pw_id	  WHERE	c.study_code IS NOT NULL AND d.pw_crf_3a_19!='1' AND		  (SELECT (((SELECT SUM(z.pw_crf4b_18c_qty) FROM form_crf_4b AS z WHERE z.study_code=ax.study_code)/(DATEDIFF(STR_TO_DATE(ax.pw_crf4b_2, '%d-%m-%Y'), STR_TO_DATE(ay.pw_crf_3a_2, '%d-%m-%Y'))  )	)*100) 		FROM 		(SELECT * FROM form_crf_4b AS xx WHERE xx.`form4b_id` IN (SELECT MAX(form4b_id) FROM form_crf_4b GROUP BY study_code)) AS ax LEFT JOIN  form_crf_3a AS ay ON ax.`study_code`=ay.`study_code` WHERE ax.study_id=c.study_id GROUP BY ax.study_id)  >=75.0  	UNION ALL SELECT 'Between 70.0% to 74.9%' AS FIELD, COUNT(c.study_code) AS 'total'		FROM 		pregnant_woman AS a LEFT JOIN  studies AS c ON c.pw_id=a.pw_id LEFT JOIN form_crf_3a AS d ON d.pw_id=a.pw_id	  WHERE c.study_code IS NOT NULL AND d.pw_crf_3a_19!='1' AND		  (SELECT (((SELECT SUM(z.pw_crf4b_18c_qty) FROM form_crf_4b AS z WHERE z.study_code=ax.study_code)/(DATEDIFF(STR_TO_DATE(ax.pw_crf4b_2, '%d-%m-%Y'), STR_TO_DATE(ay.pw_crf_3a_2, '%d-%m-%Y'))  )	)*100) 		FROM 		(SELECT * FROM form_crf_4b AS xx WHERE xx.`form4b_id` IN (SELECT MAX(form4b_id) FROM form_crf_4b GROUP BY study_code)) AS ax LEFT JOIN  form_crf_3a AS ay ON ax.`study_code`=ay.`study_code` WHERE ax.study_id=c.study_id GROUP BY ax.study_id)  BETWEEN 70.0 AND 74.9  	UNION ALL SELECT 'Between 60.0% to 69.9%' AS FIELD, COUNT(c.study_code) AS 'total'		FROM 		pregnant_woman AS a LEFT JOIN  studies AS c ON c.pw_id=a.pw_id LEFT JOIN form_crf_3a AS d ON d.pw_id=a.pw_id	  WHERE c.study_code IS NOT NULL AND d.pw_crf_3a_19!='1' AND		  (SELECT (((SELECT SUM(z.pw_crf4b_18c_qty) FROM form_crf_4b AS z WHERE z.study_code=ax.study_code)/(DATEDIFF(STR_TO_DATE(ax.pw_crf4b_2, '%d-%m-%Y'), STR_TO_DATE(ay.pw_crf_3a_2, '%d-%m-%Y'))  )	)*100) 		FROM 		(SELECT * FROM form_crf_4b AS xx WHERE xx.`form4b_id` IN (SELECT MAX(form4b_id) FROM form_crf_4b GROUP BY study_code)) AS ax LEFT JOIN  form_crf_3a AS ay ON ax.`study_code`=ay.`study_code` WHERE ax.study_id=c.study_id GROUP BY ax.study_id)  BETWEEN 60.0 AND 69.9  	UNION ALL SELECT 'Between 50.1% to 59.9%' AS FIELD, COUNT(c.study_code) AS 'total'		FROM 		pregnant_woman AS a LEFT JOIN  studies AS c ON c.pw_id=a.pw_id LEFT JOIN form_crf_3a AS d ON d.pw_id=a.pw_id	  WHERE c.study_code IS NOT NULL AND d.pw_crf_3a_19!='1' AND		  (SELECT (((SELECT SUM(z.pw_crf4b_18c_qty) FROM form_crf_4b AS z WHERE z.study_code=ax.study_code)/(DATEDIFF(STR_TO_DATE(ax.pw_crf4b_2, '%d-%m-%Y'), STR_TO_DATE(ay.pw_crf_3a_2, '%d-%m-%Y'))  )	)*100) 		FROM 		(SELECT * FROM form_crf_4b AS xx WHERE xx.`form4b_id` IN (SELECT MAX(form4b_id) FROM form_crf_4b GROUP BY study_code)) AS ax LEFT JOIN  form_crf_3a AS ay ON ax.`study_code`=ay.`study_code` WHERE ax.study_id=c.study_id GROUP BY ax.study_id)  BETWEEN 50.1 AND 59.9		UNION ALL SELECT 'Less and equal than 50.0%' AS FIELD, COUNT(c.study_code) AS 'total'		FROM 		pregnant_woman AS a LEFT JOIN  studies AS c ON c.pw_id=a.pw_id LEFT JOIN form_crf_3a AS d ON d.pw_id=a.pw_id	  WHERE c.study_code IS NOT NULL AND d.pw_crf_3a_19!='1' AND		  (SELECT (((SELECT SUM(z.pw_crf4b_18c_qty) FROM form_crf_4b AS z WHERE z.study_code=ax.study_code)/(DATEDIFF(STR_TO_DATE(ax.pw_crf4b_2, '%d-%m-%Y'), STR_TO_DATE(ay.pw_crf_3a_2, '%d-%m-%Y'))  )	)*100) 		FROM 		(SELECT * FROM form_crf_4b AS xx WHERE xx.`form4b_id` IN (SELECT MAX(form4b_id) FROM form_crf_4b GROUP BY study_code)) AS ax LEFT JOIN  form_crf_3a AS ay ON ax.`study_code`=ay.`study_code` WHERE ax.study_id=c.study_id GROUP BY ax.study_id)  <=50.0 ");
        ...
        private void ShowGraphGridViewR1()
        {
            ...
            try
            {
                con.Open();
                MySqlCommand cmd;
                cmd = new MySqlCommand("SELECT c.study_code,CONCAT(a.pw_crf_1_11,a.pw_crf_1_12,a.pw_crf_1_13,a.pw_crf_1_14,a.pw_crf_1_15,a.pw_crf_1_16) AS dssid,a.pw_crf_1_09 AS woman_nm,a.pw_crf_1_10 AS husband_nm, 				(SELECT COUNT(z.study_id) FROM form_crf_4b AS z WHERE z.study_id=c.study_id) AS CRF4b_Attempt,		(SELECT COUNT(z.study_id) FROM form_crf_4b AS z WHERE z.study_id=c.study_id AND z.pw_crf4b_14='1') AS CRF4b_Complete		,(SELECT CONCAT(ROUND ( (((SELECT SUM(z.pw_crf4b_18c_qty) FROM form_crf_4b AS z WHERE z.study_code=ax.study_code)/(DATEDIFF(STR_TO_DATE(ax.pw_crf4b_2, '%d-%m-%Y'), STR_TO_DATE(ay.pw_crf_3a_2, '%d-%m-%Y'))  )	)*100) ,1),'%')		FROM 		(SELECT * FROM form_crf_4b AS xx WHERE xx.`form4b_id` IN (SELECT MAX(form4b_id) FROM form_crf_4b GROUP BY study_code)) AS ax LEFT JOIN  form_crf_3a AS ay ON ax.`study_code`=ay.`study_code` WHERE ax.study_id=c.study_id GROUP BY ax.study_id)  AS Cumulative_Choline 		FROM 		pregnant_woman AS a LEFT JOIN  studies AS c ON c.pw_id=a.pw_id LEFT JOIN form_crf_3a AS d ON d.pw_id=a.pw_id	  WHERE c.study_code IS NOT NULL AND  d.pw_crf_3a_19='4' AND 		(SELECT (((SELECT SUM(z.pw_crf4b_18c_qty) FROM form_crf_4b AS z WHERE z.study_code=ax.study_code)/(DATEDIFF(STR_TO_DATE(ax.pw_crf4b_2, '%d-%m-%Y'), STR_TO_DATE(ay.pw_crf_3a_2, '%d-%m-%Y'))  )	)*100) 		FROM 		(SELECT * FROM form_crf_4b AS xx WHERE xx.`form4b_id` IN (SELECT MAX(form4b_id) FROM form_crf_4b GROUP BY study_code)) AS ax LEFT JOIN  form_crf_3a AS ay ON ax.`study_code`=ay.`study_code` WHERE ax.study_id=c.study_id GROUP BY ax.study_id)>=75.0		ORDER BY c.study_code;", con);
                MySqlDataAdapter sda = new MySqlDataAdapter();
                {
                    cmd.Connection = con;
                    cmd.CommandTimeout = 9999999;
