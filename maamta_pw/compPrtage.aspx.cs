    ...
    public partial class compPrtage : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                txtCalndrDate_LNS.Attributes.Add("readonly", "readonly");
                txtCalndrDate_LNS.Text = DateTime.Now.ToString("dd-MM-yyyy");
            }
        }
        ...
        private void Exportdata()
        {
            ...
            try
            {
                ...
                // (10-10-2020)      (Added         Selected Date of Cumulative Compliance)
                cmd = new MySqlCommand("SELECT  e.pw_crf_6a_19,c.study_code,CONCAT(a.pw_crf_1_11,a.pw_crf_1_12,a.pw_crf_1_13,a.pw_crf_1_14,a.pw_crf_1_15,a.pw_crf_1_16) AS dssid,a.pw_crf_1_09 AS woman_nm,a.pw_crf_1_10 AS husband_nm, d.pw_crf_3a_19 AS ARM,				(SELECT COUNT(z.study_id) FROM form_crf_4 AS z WHERE z.study_id=c.study_id) AS CRF4_Attempt,		(SELECT COUNT(z.study_id) FROM form_crf_4 AS z WHERE z.study_id=c.study_id AND z.pw_crf4_18='1') AS CRF4_Complete,TableC.last_DOV, 		  (SELECT 	CONCAT(ROUND(((SUM(z.pw_crf4_28)/SUM(		IF((z.pw_crf4_24 IS NULL || z.pw_crf4_24 =''), (SELECT (DATEDIFF(STR_TO_DATE(z.pw_crf4_2, '%d-%m-%Y'), STR_TO_DATE(y.pw_crf_3a_2, '%d-%m-%Y')  ))*2    FROM     form_crf_3a AS Y    WHERE y.study_id=z.study_id), z.pw_crf4_26)				))*100),1),'%')  FROM form_crf_4 AS z WHERE z.study_id=c.study_id AND z.pw_crf4_18='1') AS Cumulative,TableCC.last_DOV_date,  (SELECT 	CONCAT(ROUND(((SUM(z.pw_crf4_28)/SUM(		IF((z.pw_crf4_24 IS NULL || z.pw_crf4_24 =''), (SELECT (DATEDIFF(STR_TO_DATE(z.pw_crf4_2, '%d-%m-%Y'), STR_TO_DATE(y.pw_crf_3a_2, '%d-%m-%Y')  ))*2    FROM     form_crf_3a AS Y    WHERE y.study_id=z.study_id), z.pw_crf4_26)				))*100),1),'%')  FROM form_crf_4 AS z WHERE z.study_id=c.study_id AND z.pw_crf4_18='1' AND STR_TO_DATE(z.pw_crf4_2, '%d-%m-%Y')<= STR_TO_DATE('" + txtCalndrDate_LNS.Text + "', '%d-%m-%Y') ) AS Cumulative_Date,  (CASE WHEN e.`study_code`!='' THEN 'Outcome Filled' ELSE 'Ongoing' END) AS Pregnancy_Status 	,(CASE WHEN e.`study_code`!='' THEN 		CONCAT(SUBSTRING_INDEX(ABS(DATEDIFF(		STR_TO_DATE(e.pw_crf_6a_19, '%d-%m-%Y')	,ea.LMP)/7),'.',1)		,'.',    SUBSTRING_INDEX(ABS(DATEDIFF(		STR_TO_DATE(e.pw_crf_6a_19, '%d-%m-%Y')	,ea.LMP)),'.',1)-SUBSTRING_INDEX(ABS(DATEDIFF(		STR_TO_DATE(e.pw_crf_6a_19, '%d-%m-%Y')	,ea.LMP))/7,'.',1)*7) 	ELSE 			CONCAT(SUBSTRING_INDEX(ABS(DATEDIFF(		CURDATE()	,ea.LMP)/7),'.',1)		,'.',    SUBSTRING_INDEX(ABS(DATEDIFF(		CURDATE()	,ea.LMP)),'.',1)-SUBSTRING_INDEX(ABS(DATEDIFF(		CURDATE()	,ea.LMP))/7,'.',1)*7) END) AS gestational_age, 	 TableA.Consumed_LNS, (DATEDIFF(	STR_TO_DATE(TableC.last_DOV,'%d-%m-%Y'),		STR_TO_DATE(TableB.date_of_registration,'%d-%m-%Y')))*2 AS Need_to_be_used, TableB.date_of_registration,abc.*  	  FROM  (SELECT z.`study_code`,SUM(z.pw_crf4_28) AS Consumed_LNS FROM form_crf_4 AS z WHERE z.pw_crf4_22b!=1 GROUP BY z.study_code ) AS TableA  LEFT JOIN  (SELECT study_code,DATE_FORMAT(STR_TO_DATE(pw_crf_3a_2,'%d-%m-%Y'),'%d-%m-%Y')  AS date_of_registration FROM form_crf_3a) AS TableB ON TableA.study_code=TableB.study_code LEFT JOIN  (SELECT study_code, DATE_FORMAT(MAX(STR_TO_DATE(pw_crf4_2,'%d-%m-%Y')),'%d-%m-%Y') AS last_DOV FROM form_crf_4 WHERE pw_crf4_22b!=1   GROUP BY study_code)AS TableC ON TableA.study_code=TableC.study_code LEFT JOIN   studies AS c ON c.study_code=TableA.study_code LEFT JOIN   pregnant_woman AS a ON c.pw_id=a.pw_id  LEFT JOIN form_crf_3a AS d ON d.pw_id=a.pw_id	 LEFT JOIN form_crf_6a AS e ON e.`study_code`=d.`study_code`   	 LEFT JOIN (SELECT * FROM (SELECT b.form_crf_1_id,b.`pw_id`,b.`pw_assist_code`,	 DATE_SUB((STR_TO_DATE(b.pw_crf1_02, '%d-%m-%Y')), INTERVAL ((c.pw_crf_1_30_week*7)+c.pw_crf_1_30_days)  DAY) AS LMP  FROM form_crf_1 AS b LEFT JOIN ultrasound_examination AS c ON c.form_crf1_id=b.form_crf_1_id ) AS a WHERE a.form_crf_1_id IN (SELECT MIN(z.form_crf_1_id) FROM form_crf_1 AS z GROUP BY z.pw_assist_code)) AS ea ON ea.pw_id=a.pw_id      				LEFT JOIN 	view_crf4_flat_status AS abc ON abc.study_code=c.study_code 			  LEFT JOIN  (SELECT study_code, DATE_FORMAT(MAX(STR_TO_DATE(pw_crf4_2,'%d-%m-%Y')),'%d-%m-%Y') AS last_DOV_date FROM form_crf_4 WHERE pw_crf4_22b!=1 AND STR_TO_DATE(pw_crf4_2, '%d-%m-%Y')<= STR_TO_DATE('" + txtCalndrDate_LNS.Text + "', '%d-%m-%Y') 	  GROUP BY study_code)AS TableCC ON TableA.study_code=TableCC.study_code  WHERE c.study_code IS NOT NULL AND d.pw_crf_3a_19!='1' AND CONCAT(a.pw_crf_1_11,a.pw_crf_1_12,a.pw_crf_1_13,a.pw_crf_1_14,a.pw_crf_1_15,a.pw_crf_1_16) LIKE '%" + txtdssid.Text.ToUpper() + "%' ORDER BY c.study_code;", con);

                // (07-10-2020)      (Added         Followup Status Flat file)
                // cmd = new MySqlCommand("SELECT  e.pw_crf_6a_19,c.study_code,CONCAT(a.pw_crf_1_11,a.pw_crf_1_12,a.pw_crf_1_13,a.pw_crf_1_14,a.pw_crf_1_15,a.pw_crf_1_16) AS dssid,a.pw_crf_1_09 AS woman_nm,a.pw_crf_1_10 AS husband_nm, d.pw_crf_3a_19 AS ARM,				(SELECT COUNT(z.study_id) FROM form_crf_4 AS z WHERE z.study_id=c.study_id) AS CRF4_Attempt,		(SELECT COUNT(z.study_id) FROM form_crf_4 AS z WHERE z.study_id=c.study_id AND z.pw_crf4_18='1') AS CRF4_Complete, 		 (SELECT 	CONCAT(ROUND(((SUM(z.pw_crf4_28)/SUM(		IF((z.pw_crf4_24 IS NULL || z.pw_crf4_24 =''), (SELECT (DATEDIFF(STR_TO_DATE(z.pw_crf4_2, '%d-%m-%Y'), STR_TO_DATE(y.pw_crf_3a_2, '%d-%m-%Y')  ))*2    FROM     form_crf_3a AS Y    WHERE y.study_id=z.study_id), z.pw_crf4_26)				))*100),1),'%')  FROM form_crf_4 AS z WHERE z.study_id=c.study_id AND z.pw_crf4_18='1') AS Cumulative, (CASE WHEN e.`study_code`!='' THEN 'Outcome Filled' ELSE 'Ongoing' END) AS Pregnancy_Status 	,(CASE WHEN e.`study_code`!='' THEN 		CONCAT(SUBSTRING_INDEX(ABS(DATEDIFF(		STR_TO_DATE(e.pw_crf_6a_19, '%d-%m-%Y')	,ea.LMP)/7),'.',1)		,'.',    SUBSTRING_INDEX(ABS(DATEDIFF(		STR_TO_DATE(e.pw_crf_6a_19, '%d-%m-%Y')	,ea.LMP)),'.',1)-SUBSTRING_INDEX(ABS(DATEDIFF(		STR_TO_DATE(e.pw_crf_6a_19, '%d-%m-%Y')	,ea.LMP))/7,'.',1)*7) 	ELSE 			CONCAT(SUBSTRING_INDEX(ABS(DATEDIFF(		CURDATE()	,ea.LMP)/7),'.',1)		,'.',    SUBSTRING_INDEX(ABS(DATEDIFF(		CURDATE()	,ea.LMP)),'.',1)-SUBSTRING_INDEX(ABS(DATEDIFF(		CURDATE()	,ea.LMP))/7,'.',1)*7) END) AS gestational_age, 	 TableA.Consumed_LNS, (DATEDIFF(	STR_TO_DATE(TableC.last_DOV,'%d-%m-%Y'),		STR_TO_DATE(TableB.date_of_registration,'%d-%m-%Y')))*2 AS Need_to_be_used, TableB.date_of_registration,TableC.last_DOV FROM  (SELECT z.`study_code`,SUM(z.pw_crf4_28) AS Consumed_LNS FROM form_crf_4 AS z WHERE z.pw_crf4_22b!=1 GROUP BY z.study_code ) AS TableA  LEFT JOIN  (SELECT study_code,DATE_FORMAT(STR_TO_DATE(pw_crf_3a_2,'%d-%m-%Y'),'%d-%m-%Y')  AS date_of_registration FROM form_crf_3a) AS TableB ON TableA.study_code=TableB.study_code LEFT JOIN  (SELECT study_code, DATE_FORMAT(MAX(STR_TO_DATE(pw_crf4_2,'%d-%m-%Y')),'%d-%m-%Y') AS last_DOV FROM form_crf_4 WHERE pw_crf4_22b!=1   GROUP BY study_code)AS TableC ON TableA.study_code=TableC.study_code LEFT JOIN   studies AS c ON c.study_code=TableA.study_code LEFT JOIN   pregnant_woman AS a ON c.pw_id=a.pw_id  LEFT JOIN form_crf_3a AS d ON d.pw_id=a.pw_id	 LEFT JOIN form_crf_6a AS e ON e.`study_code`=d.`study_code`   	 LEFT JOIN (SELECT * FROM (SELECT b.form_crf_1_id,b.`pw_id`,b.`pw_assist_code`,	 DATE_SUB((STR_TO_DATE(b.pw_crf1_02, '%d-%m-%Y')), INTERVAL ((c.pw_crf_1_30_week*7)+c.pw_crf_1_30_days)  DAY) AS LMP  FROM form_crf_1 AS b LEFT JOIN ultrasound_examination AS c ON c.form_crf1_id=b.form_crf_1_id ) AS a WHERE a.form_crf_1_id IN (SELECT MIN(z.form_crf_1_id) FROM form_crf_1 AS z GROUP BY z.pw_assist_code)) AS ea ON ea.pw_id=a.pw_id      	 WHERE c.study_code IS NOT NULL AND d.pw_crf_3a_19!='1' AND CONCAT(a.pw_crf_1_11,a.pw_crf_1_12,a.pw_crf_1_13,a.pw_crf_1_14,a.pw_crf_1_15,a.pw_crf_1_16) LIKE '%" + txtdssid.Text.ToUpper() + "%' ORDER BY c.study_code;", con);


                // (02-09-2020)      Added          Consumed Sachet / Need to be used / Last DOV / DOR 
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=LNS Cumulative (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
