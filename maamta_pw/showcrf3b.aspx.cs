...
namespace maamta_pw
{
    public partial class showcrf3b : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            try
            {
                if (CheckBox1.Checked == false)
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf3b WHERE DSSID LIKE '%" + txtdssid.Text + "%' and (str_to_date(pw_crf3b_2, '%d-%m-%Y') between str_to_date('" + txtCalndrDate.Text + "', '%d-%m-%Y') and str_to_date('" + txtCalndrDate1.Text + "', '%d-%m-%Y'))       order by str_to_date(pw_crf3b_2, '%d-%m-%Y'), STR_TO_DATE(pw_crf3b_3,  '%H:%i')", con);
                ...
                else
                {
                    con.Open();
                    MySqlCommand cmd;
                    cmd = new MySqlCommand("select * from view_crf3b WHERE DSSID LIKE '%" + txtdssid.Text + "%'        order by str_to_date(pw_crf3b_2, '%d-%m-%Y'), STR_TO_DATE(pw_crf3b_3,  '%H:%i')", con);
                    MySqlDataAdapter sda = new MySqlDataAdapter();
                    {
                        cmd.Connection = con;
                        sda.SelectCommand = cmd;
        ...
        protected void Link_StudyID(object sender, EventArgs e)
        {
            ...
            Session["form_crf_3b_id"] = form_crf_3a_id;
            Session["StudyIdCRF3b"] = StudyId;
            Session["BackButtonCRF3b"] = "showcrf3b";
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=CRF3b (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
