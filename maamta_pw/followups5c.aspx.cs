...
namespace maamta_pw
{
    public partial class followups5c : System.Web.UI.Page
    {
        ...
        private void ShowData()
        {
            ...
            try
            {
                ...
                if (CheckBox1.Checked == true)
                {
                    ...
                    //else
                    //{
                    cmd = new MySqlCommand("select followup_id, site,SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 3), ':', -1)  AS block, followup_no, pw_assid, study_id, concat(SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 1), ':', -1), SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 2), ':', -1) , SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 3), ':', -1) , SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 4), ':', -1) , SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 5), ':', -1) , SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 6), ':', -1) ) as dssid,  pw_name,husband_name, (CASE WHEN arm= '1' THEN 'A' WHEN arm = '2' THEN 'B' WHEN arm = '3' THEN 'C' WHEN arm = '4' THEN 'D'  END) as ARM,start_date, DAYNAME(str_to_date(start_date, '%d-%m-%Y')) as Day, DATE_FORMAT(DATE_SUB(str_to_date(end_date, '%d-%m-%Y'), interval 1 day), '%d-%m-%Y') as end_date, status from followups where form='5c' and status='3'  and  str_to_date(start_date, '%d-%m-%Y') <= CURDATE()  and concat(SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 1), ':', -1), SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 2), ':', -1) , SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 3), ':', -1) , SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 4), ':', -1) , SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 5), ':', -1) , SUBSTRING_INDEX(SUBSTRING_INDEX(dss_id, ':', 6), ':', -1) ) like '%" + txtdssid.Text + "%' order by study_id, followup_id ", con);
                        MySqlDataAdapter sda = new MySqlDataAdapter();
                        {
                            cmd.Connection = con;
                            sda.SelectCommand = cmd;
        ...
        public void ExcelExport()
        {
            ...
            {
                Response.Clear();
                Response.AddHeader("content-disposition", "attachment;filename=PW_Followup-5c (" + DateTime.Today.ToString("dd-MM-yyyy") + ").xls");
                Response.Charset = "";
